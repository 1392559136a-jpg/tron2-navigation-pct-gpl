...
class PCTNavigationNode(Node):
    ...
    def __init__(self, scene, tomogram_override=''):
        ...
        self.stair_centerline_enabled = bool(
            self.declare_parameter('stair_centerline.enabled', False).value
        )
        self.stair_centerline_max_cost = float(
            self.declare_parameter('stair_centerline.max_cost', 20.0).value
        )
        self.stair_centerline_max_support_error = float(
            self.declare_parameter(
                'stair_centerline.max_support_error', 0.25
            ).value
        )
        self.stair_centerline_max_support_step = float(
            self.declare_parameter(
                'stair_centerline.max_support_step', 0.30
            ).value
        )
        self.stair_centerline_max_layer_step = int(
            self.declare_parameter('stair_centerline.max_layer_step', 1).value
        )
        self.stair_centerline_smoothing_window = int(
            self.declare_parameter('stair_centerline.smoothing_window', 9).value
        )
        self.stair_centerline_minimum_xy_length = float(
            self.declare_parameter(
                'stair_centerline.minimum_xy_length', 0.5
            ).value
        )
        self.stair_centerline_minimum_vertical_span = float(
            self.declare_parameter(
        ...
        if self.stair_centerline_enabled:
            self.get_logger().warning(
                'STAIR CENTERLINE MODE enabled: PCT A* is bypassed; only a '
                'validated XY-straight path will be published'
            )
        self.get_logger().info(
            'Ready: pose=%s goals=[%s, %s] path=%s frame=%s map=%s centerline=%s'
            % (
                self.pose_topic,
                self.goal_topic,
                self.legacy_goal_topic,
    ...
    def goal_callback(self, goal):
        ...
        try:
            if self.stair_centerline_enabled:
                trajectory = self.planner.plan_stair_centerline(
                    start_pos,
                    goal_pos,
                    start_layer=requested_start_layer,
                    end_layer=requested_goal_layer,
                    max_cost=self.stair_centerline_max_cost,
                    max_support_error=self.stair_centerline_max_support_error,
                    max_support_step=self.stair_centerline_max_support_step,
                    max_layer_step=self.stair_centerline_max_layer_step,
                    smoothing_window=self.stair_centerline_smoothing_window,
                    minimum_xy_length=self.stair_centerline_minimum_xy_length,
                    minimum_vertical_span=(
                        self.stair_centerline_minimum_vertical_span
                    ),
                )
            else:
        ...
        if self.stair_centerline_enabled:
            result = self.planner.last_centerline_result
            self.get_logger().info(
                'Published validated stair centerline: %d points, %.2fm, '
                'layers %d -> %d, max cost %.3f, support error %.3fm'
                % (
                    len(path.poses),
                    result.length,
