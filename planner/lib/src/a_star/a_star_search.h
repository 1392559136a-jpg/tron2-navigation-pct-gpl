...
class Node {
 public:
  ...
  bool operator==(const Node& other) const {
    return layer == other.layer && idx[1] == other.idx[1] &&
           idx[2] == other.idx[2];
  }
  ...
  double cost = 0.0;
  int layer = 0;
  Eigen::Vector3i idx = Eigen::Vector3i(0, 0, 0);  // height cell, row, col
  Node* parent = nullptr;
};
...
class Astar {
 ...
 private:
  ...
  std::vector<int> DecideLayers(const Node* cur_node) const;

  std::size_t GetHash(const Node* node) const;
