...
std::size_t Astar::GetHash(const Node* node) const {
  return static_cast<std::size_t>(node->layer) * xy_size_ +
         static_cast<std::size_t>(node->idx[1]) * max_x_ + node->idx[2];
}

bool Astar::Search(const Eigen::Vector3i& start, const Eigen::Vector3i& goal) {
  auto t0 = std::chrono::high_resolution_clock::now();

  // A failed search leaves g/f/parent values in the grid while producing no
  // search_result_. Reset after every prior attempt, not just after success.
  if (has_searched_) {
    Reset();
  }
  has_searched_ = true;
  ...
  std::unordered_map<std::size_t, Node*> closed_set;
  ...
  while (!open_set.empty()) {
    ...
    // A tomogram may represent the same physical elevation in several slices.
    // Layer, row and column together identify a search state; elevation does
    // not. Pointer equality is exact because every state owns one grid node.
    ...
    for (const int layer : DecideLayers(current_node)) {
...
std::vector<int> Astar::DecideLayers(const Node* cur_node) const {
  int layer = cur_node->layer;
  int i = cur_node->idx[1];
  int j = cur_node->idx[2];
  double cur_height = cur_node->height;

  std::vector<int> layers{layer};
  auto add_layer = [&layers](int candidate) {
    if (std::find(layers.begin(), layers.end(), candidate) == layers.end()) {
      layers.emplace_back(candidate);
    }
  };

  // A gateway may be encoded on either endpoint of a layer boundary. Explore
  // both valid directions instead of forcing one signed value to represent a
  // potentially bidirectional transition.
  if (layer + 1 < max_layers_) {
    const Node& upper = grid_map_[layer + 1][i][j];
    if (abs(upper.height - cur_height) <= 0.2 &&
        (cur_node->ele > 0.5 || upper.ele < -0.5)) {
      add_layer(layer + 1);
    }
  }
  if (layer - 1 >= 0) {
    const Node& lower = grid_map_[layer - 1][i][j];
    if (abs(lower.height - cur_height) <= 0.2 &&
        (cur_node->ele < -0.5 || lower.ele > 0.5)) {
      add_layer(layer - 1);
    }
  }
...
void Astar::ConvertClosedSetToMatrix(
  const std::unordered_map<std::size_t, Node*>& closed_set) {
