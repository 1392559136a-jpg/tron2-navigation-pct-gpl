...
bool GPMPOptimizer::GenerateTrajectory(const std::vector<PathPoint>& input_path,
  ...
  for (int i = 0; i < N; ++i) {
    ...
    if (i < N - 1) {
      ...
      for (int j = 0; j < interpolate_num_; ++j) {
        ...
        if (debug_) {
          printf(
              "path[%d/%lld], layer: %d, inter_layer: %f, height: %f, "
              "height2:%f, hint%f, (%f, %f)\n",
              i, static_cast<long long>(opt_init_layer_.size() - 1),
              path[i].layer, opt_init_layer_(col_index - 1), path[i].height,
              path[i + 1].height,
  ...
  trajectory_ = traj_interpolator.GenerateTrajectory(interpolate_num_);
    printf("shape: %lld, %lld\n", static_cast<long long>(trajectory_.rows()),
      static_cast<long long>(trajectory_.cols()));
